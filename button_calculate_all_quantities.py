...
class qtoAllQuantitiesCalculator():
    ...
    def calculate_all_qtos(self, selected_objects):
        ...
        for object in selected_objects:
            ...
            self.pset_qto_ifc_instance = self.get_pset_qto_ifc_instance(object)
            if not self.pset_qto_ifc_instance:
            ...
            for pset_qto_property in self.pset_qto_properties:
                ...
                self.pset_qto_id = self.get_pset_qto_id(object)
    ...
    def get_pset_qto_ifc_instance(self, object):
    ...
    def get_pset_qto_id(self, object):
        pset_qto_name = self.get_pset_qto_name(object)
        pset_qto_id = self.file.by_id(self.pset_qto_ifc_instance[pset_qto_name]['id']) #TODO check if i can use line above
        return pset_qto_id
    ...
    def edit_qto(self, object, quantity_name, new_quantity):
        ...
        ifcopenshell.api.run("pset.edit_qto",
                self.file,
                **{"qto" : self.pset_qto_id, "name" : pset_qto_name, "properties": {quantity_name : new_quantity}}
            )
